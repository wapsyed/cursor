...
-   **Análise Exploratória de Dados:** Realizaremos análises exploratórias de dados, incluindo estatísticas descritivas, gráficos e interpretação de resultados. Aplicaremos os conhecimentos adquiridos em um projeto prático com dados reais.
...
library(ggpubr) #Funções interessantes para publicações
library(scales) #Lidar com números, estatísticas e escalas
library(here) #Acessar diretórios rapidamente
library(readxl) #Importar arquivos de excel
```
...
data %>%                   # Tenho este objeto 
  filter(variavel2 >= 30) %>%  # E então vou filtrar os valores maiores de 30
  filter(gene == "Gene1")   # E então vou filtrar os indivíduos chamados "Gene1"
```
...
#Vamos duplicar o objeto "data" como "data_2", para evitar de sobrescrever a tabela original.
data_2 = data
...
A função `mutate()` permite **criar ou modificar colunas** e imputar valores usando funções que se aplicarão a todas as linhas daquela coluna. Por exemplo, podemos criar a coluna factor com um fator arbitrário de todos os indivíduos, que é calculado com a razão da variável 2 e a variável 2 ao quadrado.
...
Por exemplo, podemos criar uma coluna que classificará se a pessoa está acima ou abaixo de um fator específico, a partir dos valores da coluna `imc`.

Isso é feito com a função `if_else(),` que é lida da seguinte forma: se isso for verdadeiro, dê o valor "A", do contrário, "B". No caso aqui, se a pessoa tiver um fator acima de 10, ela será considerada com super, e se for abaixo, será classificada como normal.
...
Mas, podemos classificar com outras categorias. Para isso, temos o `case_when()`, que é um if_else mais versátil e deixa o código mais limpo e fácil de ler.
...
linha = data_2 %>% 
  ...
  aes(x = variavel2,
      ...
      #Vamos add um label para identificar os genes em cada ponto
      label = gene
      ) +
  ...
  #Títulos
    labs(x = "Variable 2", 
       y = "Factor",
       title = "Factor versus Variable 2",
       color = "Classification") +
...
```{r}
linha %>% 
  ggsave(file = here("Bioinformática","linha_factor.png"),
         width = 5, 
         height = 5)
```
...
```{r}
metadata <- read_csv(here("Bioinformática", "metadata.csv"))
expression_data <- read_csv(here("Bioinformática", "expression_data.csv"))
```
...
###Boxplot com violin plot e pontos
...
### Heatmap
Para criar o heatmap no ggplot, é necessário pivotar para o formato longo, com uma coluna para genes, outra para o identificador da amostra, e finalmente uma coluna com os valores de expressão do gene correspondente.
Como as amostras estão estratificadas por condição, a visualização se torna mais clara quando adicionamos a condição como metadado. Com dados reais, podemos adicionar quantos metadados forem necessários, mas é importante tomar cuidado com a ordem deles. 
...
#Unir tabelas
heatmap_table_annotated = heatmap_table %>% 
  inner_join(annotation_col,
             by = "sample")

#Plotar heatmap
heatmap_table_annotated %>% 
  ggplot() +
  aes(x = sample,
      y = gene,
      fill = expression) +
...
Em outros pacotes, como o complexheatmap, do Bioconductor, as funções nativas para plotagem já lidam com a clusterização e outros parametros legais, como os dendrogramas. Entretanto, o posit cloud tem muita dificuldade de trabalhar com os pacotes do bioconductor, então optei por seguir com as opções do ggplot2. Em essência, não muda muito do que estamos fazendo aqui! 

Abaixo, para clusterizarmos, utilizamos a matriz original, calculamos a distância e o agrupamento por genes e depois por amostras, aplicamos a ordem na tabela do heatmap input do ggplot, e finalmente plotamos nosso heatmap!

```{r fig.height=10, fig.width=10}
#Crie a tabela no formato de matriz (wide)
matrix_data = expression_data %>% 
  column_to_rownames("gene")

#Calcule a distância e o agrupamento para GENES (linhas)
gene_dist <- dist(matrix_data)
gene_hclust <- hclust(gene_dist)
gene_order <- rownames(matrix_data)[gene_hclust$order]

#Calcule a distância e o agrupamento para SAMPLES (colunas)
sample_dist <- dist(t(matrix_data)) # t() para transpor e agrupar colunas
sample_hclust <- hclust(sample_dist)
sample_order <- colnames(matrix_data)[sample_hclust$order]

#Reordene os genes e amostras
heatmap_table_annotated_ordered = heatmap_table_annotated %>% 
  mutate(gene = fct_relevel(gene, gene_order),
         sample = fct_relevel(sample, sample_order))

#Gere o heatmap
heatmap_table_annotated_ordered %>% 
  ggplot() +
  aes(x = sample,
      y = gene,
      fill = expression) +
  ...
  geom_tile() +
  theme(axis.text.x = element_text(angle = 90)) +
  #Mude a escala de cores
  scale_fill_viridis_c(
    option = "mako",
    direction = 1,
    limits = c(0, 20),           # Define o limites
    oob = scales::squish         # "Espremer" os valores > limite para a cor máxima
  )
```
...
-   **Use a IA.** O ChatGPT é uma ferramenta extremamente útil para a sua evolução na programação e você precisa aprender a usá-lo e a fazer perguntas corretas. 80% das vezes ele dá respostas perfeitas. Você pode usá-lo para explicar, consertar e criar códigos!
