...
ELISA_Adri <- read_csv(here("Gráficos diversos", "Adriana", "ELISA_Adri.csv"),
                       col_types = cols(Diluição = col_character()))

table = ELISA_Adri %>% 
  pivot_longer(cols = -"Diluição",
               values_to = "value",
               names_to = "group") %>% 
  ...
  mutate(value = str_replace(value, ",", ".") %>% 
           as.numeric(),
         diluicao = fct_relevel(diluicao, c(ELISA_Adri$Diluição))) %>% 
  separate(group, into = c("grupo", "tipo", "m"), remove = F) %>% 
  mutate(grupo = paste(grupo, tipo, sep = " "),
         m = if_else(is.na(m), "NA", m)) %>% 
  group_by(diluicao, grupo, m) %>% 
...
table %>%
  ...
  # scale_shape_manual(values = c(15, 16, 17, 23, 19, 20, 0, 1, 2)) + #Shapes preenchidos
  scale_shape_manual(values = c(0, 1, 2, 5, 6, 8)) + #Shapes vazios
