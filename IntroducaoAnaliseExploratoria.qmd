...
> 1.  **Para encontrar este documento com as linhas de códigos**, clique no arquivo **QMD** "**`Introducao_analise_exploratoria.rmd`**". Se clicar no `.md`, não funcionará.
...
```{r paged.print=TRUE}
#Pelo readr
cobertura_vacinal <- read_csv(here("cobertura_vacinal_anotada.csv"))  

cobertura_vacinal
```

Para baixar uma tabela de outra pasta, podemos usar a função `here()` do pacote `here`. Como dito anteriormente, essa função sempre assumirá que o diretório inicial é o do projeto. Para acessar um arquivo dentro de uma pasta, no caso o arquivo `paises_anotados.rds` na pasta `raw-data`, escrevemos esse caminho entre aspas.
...
mortes_doencas <- read_rds(here("Deaths_infectious_diseases_filtered.rds")) 
...
#Salvar
cobertura_vacinal %>% 
  saveRDS(file = here("cobertura_vacinal_anotada_2.rds"))
```
...
#Salvar
mortes_doencas %>% 
  saveRDS(file = here("mortes_doencas.rds"))
