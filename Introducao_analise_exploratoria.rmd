...
continentes_vac = cobertura_vacinal %>% 
  ...
  group_by(vaccine, region_complete) %>% 
...
continentes_mortes = mortes_doencas %>% 
  ...
  group_by(death_disease, region_complete) %>%
...
continentes_pontos_vac = continentes_vac %>% #Dataframe
  ...
  mutate(region_complete = fct_reorder(region_complete, median)) %>% 
...
barras_mortes = continentes_mortes %>% #Dataframe
  ...
  mutate(region_complete = fct_reorder(region_complete, cumulativo)) %>% 
  ggplot() + #Chamando a função. Aqui se usa "+" em vez de "%>%" 
...
boxplot_regions = cobertura_vacinal %>% 
 ...
 ggplot() +
  ...
  theme_minimal() +
  facet_wrap(vars(region_complete)) +
  scale_fill_npg()
...
cobertura_vacinal %>% 
  filter(type == "country",
         year == 2015) %>% 
 ggplot() +
  aes(x = "", y = coverage, fill = region_complete) +
  geom_boxplot() +
...
# 7. Extra: Analisando distribuições e uma pincelada de estatística
...
cobertura_vacinal %>%
 filter(type == "country") %>% 
  select(coverage, year, region_complete, vaccine) %>% 
  mutate(region_complete = fct_reorder(region_complete, coverage)) %>% 
  drop_na(coverage) %>% 
  ggplot() +
  aes(x = year, fill = region_complete) +
...
cobertura_vacinal %>%
 filter(type == "country",
        year %in% c(1990,2000, 2010, 2020),
        diseases == "Measles") %>%
  mutate(region_complete = fct_rev(region_complete)) %>% 
 ggplot() +
  aes(x = coverage, y = region_complete, fill = fct_rev(region_complete)) +
   geom_density_ridges() +
  theme_few()+
...
# 8. Extra: Dados demográficos e economicos com o Gapminder
...
```{r message=FALSE, warning=FALSE}
gapminder %>% 
  left_join(cobertura_vacinal %>% rename(country = name),
            by = join_by(country, year)) %>% 
  filter(vaccine == "BCG",
         region_complete != "Non-programme") %>% 
ggplot() +
  aes(
    x = coverage,
    y = lifeExp,
  ) +
  geom_point(aes(colour = region_complete,
    size = pop),
    shape = "circle") +
  facet_wrap(~year)+
...
Observe que o maior valor de R foi de 0.61 e de R^2^, de 0.37. Somente de observar os gráficos, vemos que esses resultados corroboram com a dispersão dos países, com muitos nas extremidades dos gráficos. Neste caso, podemos dizer que, apesar de uma associação entre as variáveis, a correlação é positiva e intermediária. Além disso, a expectativa de vida somente explica 37% da variação da cobertura vacinal entre os países.
...
Ao usarmos a regressão linear com todos os países, desconsideramos os **agrupamentos** dados pelas regiões da OMS. Afinal, cada região tem variabilidade entre seus países, e na média, uma região pode ter um comportamento diferente da outra. Podemos ver isso fazendo uma regressão linear por região e por ano. Observe que, ao d
...
ggplot() +
  ...
  geom_point(shape = "circle",
             aes(colour = region_complete,
                 size = pop),
             alpha = 0.2) +
  geom_smooth(aes(group = region_complete,
                  color = region_complete),
              method = "lm",
              se = F)+
  stat_correlation(method = "pearson",
