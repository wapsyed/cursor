...
tabelageralunicef %>% 
  mutate(unicef_region = if_else(is.na(unicef_region), region, unicef_region),
         ...
         iso3 = if_else(is.na(country), region, iso3)) %>% 
...
Deaths_infectious_diseases_filtered = Deaths_infectious_diseases %>% 
  ...
  select(who_region_name, who_region_code, name, year, total_deaths:death_disease) 
...
data_long_annotated = data_long_standardized %>% 
  ...
  inner_join(Deaths_infectious_diseases_filtered,
...
#Salvar
ggsave(plot, file = "sarampo_lacr_2000_2023.png", width = 10, height = 5)
```
