...
filter(data, comparacao)

data %>% filter(comparacao)
...
gabriela = data %>%                   # Tenho este objeto 
  filter(idade >= 30) %>%  # E então vou filtrar os indivíduos com menos de 30 anos
  filter(nome == "Gabriela")   # E então vou filtrar os indivíduos que não incluam a "Júlia"
```
...
data_long %>% 
  mutate(regiao_completa = if_else(unicef_region == "ROSA",
                                   "South Asia",
                                   NA),
         regiao_completa = case_when(unicef_region == "EAPR" ~ "East Asia and Pacific", 
                              unicef_region == "ECAR" ~ "Europe and Central Asia",
                              unicef_region == "MENA" ~ "Middle East and North Africa",
                              unicef_region == "ESAR" ~ "Eastern and Southern Africa",
                              unicef_region == "LACR" ~ "Latin America and Caribbean",
                              unicef_region == "Non-programme" ~ "Non-programme",
                              unicef_region == "ROSA" ~ "South Asia",
                              unicef_region == "WCAR" ~ "West and Central Africa",
                              TRUE ~ unicef_region))
