...
Para salvar uma figura, vamos usar a função `ggsave()` e definir como tipo de arquivo `.png`. Há diferentes tipos de arquivos, como jpeg e tiff, mas usaremos o png por ser muito comum e também ser aceito por diferentes programas de edição, como o Google Slides, Powerpoint e Canva. Além disso, vamos definir também as dimensões da imagem final.
...
ggsave(linha_barra, file = "linha_barra.png", width = 10, height = 5)
```
...
Para baixar uma tabela de outra pasta, podemos usar a função `here()` do pacote `here`. Essa função sempre assumirá que o diretório inicial é o do projeto. Para acessar um arquivo dentro de uma pasta, no caso o arquivo `paises_anotados.rds` na pasta `raw-data`, escrevemos esse caminho entre aspas.
...
Testando correlações entre mais variáveis
