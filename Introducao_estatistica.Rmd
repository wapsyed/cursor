...
normal_dist_thousand = rnorm(1000, mean = 30, sd = 1) %>% 
  as.data.frame() %>% 
  rename(distribution = ".") %>% 
  mutate(group = "G1")

normal_dist_params_thousand = normal_dist_thousand %>% 
  summarize(mean = mean(distribution),
            median = median(distribution))
...
patch_normal_dist_many

patch_normal_dist_few | patch_normal_dist_many 
```
# Comparando muitas e poucas amostras (desvio padrão grande)
## Um grupo
